    ...
    <main className="container mx-auto px-4 py-12">
      ...
      {announcements.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-djkn-600">Belum ada pengumuman yang tersedia saat ini.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {announcements.map((item) => (
            <Card key={item.id} className="overflow-hidden flex flex-col hover:shadow-lg transition-shadow border-djkn-100">
              {item.image_url && (
